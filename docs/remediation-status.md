# Remediation status

States are evidence-bound. `fixed-and-tested` needs code plus a passing reproducible check. Local proof does not imply release proof.

## Critical

| ID | State |
| --- | --- |
| C-01 | blocked |
| C-02 | in-progress |
| C-03 | fixed-and-tested |
| C-04 | in-progress |
| C-05 | not-started |
| C-06 | not-started |
| C-07 | not-started |
| C-08 | not-started |
| C-09 | in-progress |
| C-10 | not-started |
| C-11 | in-progress |
| C-12 | not-started |

## High

| ID | State |
| --- | --- |
| H-01 | not-started |
| H-02 | not-started |
| H-03 | not-started |
| H-04 | not-started |
| H-05 | not-started |
| H-06 | not-started |
| H-07 | not-started |
| H-08 | not-started |
| H-09 | in-progress |
| H-10 | in-progress |
| H-11 | fixed-and-tested |
| H-12 | fixed-and-tested |
| H-13 | in-progress |
| H-14 | fixed-and-tested |
| H-15 | in-progress |
| H-16 | not-started |
| H-17 | not-started |
| H-18 | not-started |
| H-19 | not-started |
| H-20 | not-started |
| H-21 | not-started |
| H-22 | not-started |
| H-23 | not-started |
| H-24 | in-progress |
| H-25 | not-started |
| H-26 | fixed-and-tested |
| H-27 | fixed-and-tested |
| H-28 | fixed-and-tested |
| H-29 | not-started |
| H-30 | in-progress |
| H-31 | not-started |
| H-32 | in-progress |

## Medium

| ID | State |
| --- | --- |
| M-01 | not-started |
| M-02 | not-started |
| M-03 | not-started |
| M-04 | fixed-and-tested |
| M-05 | not-started |
| M-06 | fixed-and-tested |
| M-07 | fixed-and-tested |
| M-08 | not-started |
| M-09 | not-started |
| M-10 | not-started |
| M-11 | not-started |
| M-12 | not-started |
| M-13 | not-started |
| M-14 | not-started |
| M-15 | not-started |
| M-16 | not-started |
| M-17 | not-started |
| M-18 | fixed-and-tested |
| M-19 | not-started |
| M-20 | not-started |
| M-21 | not-started |
| M-22 | not-started |
| M-23 | not-started |
| M-24 | not-started |
| M-25 | not-started |
| M-26 | not-started |
| M-27 | not-started |
| M-28 | not-started |
| M-29 | not-started |
| M-30 | fixed-and-tested |
| M-31 | not-started |
| M-32 | not-started |
| M-33 | not-started |
| M-34 | not-started |
| M-35 | not-started |
| M-36 | not-started |
| M-37 | not-started |
| M-38 | not-started |
| M-39 | not-started |
| M-40 | not-started |
| M-41 | not-started |
| M-42 | not-started |
| M-43 | not-started |
| M-44 | not-started |
| M-45 | not-started |
| M-46 | not-started |
| M-47 | not-started |
| M-48 | in-progress |
| M-49 | not-started |
| M-50 | not-started |
| M-51 | not-started |
| M-52 | not-started |
| M-53 | not-started |
| M-54 | fixed-and-tested |
| M-55 | fixed-and-tested |
| M-56 | not-started |
| M-57 | not-started |
| M-58 | fixed-and-tested |
| M-59 | not-started |
| M-60 | not-started |
| M-61 | not-started |
| M-62 | not-started |
| M-63 | not-started |
| M-64 | not-started |
| M-65 | fixed-and-tested |
| M-66 | in-progress |
| M-67 | fixed-and-tested |
| M-68 | fixed-and-tested |
| M-69 | not-started |
| M-70 | fixed-and-tested |
| M-71 | fixed-and-tested |
| M-72 | in-progress |
| M-73 | fixed-and-tested |
| M-74 | not-started |

## Low

| ID | State |
| --- | --- |
| L-01 | not-started |
| L-02 | not-started |
| L-03 | not-started |
| L-04 | not-started |
| L-05 | not-started |
| L-06 | not-started |
| L-07 | not-started |
| L-08 | fixed-and-tested |

## Current phase

Phase 0 local work is complete. Production release remains blocked by history findings, operator incident work, Docker proof, and disposable PostgreSQL proof. Phase 1 is in progress.
