...
export default function BottomNav() {
  ...
  // Navigation items structured exactly as the PNG design
  const navItems = [
    ...
    { name: "اکتشاف", path: "/products", icon: <Search className="w-[22px] h-[22px]" /> },
    { name: "خانه", path: "/", icon: <Home className="w-[22px] h-[22px]" /> },
  ];

  return (
    <div className="fixed bottom-0 left-0 w-full bg-[#0f0f13]/95 backdrop-blur-xl border-t border-zinc-800/60 z-50 pb-2">
          ...
          return (
            <Link 
              ...
              className={`flex flex-col items-center justify-center gap-1.5 transition-all duration-300 ease-in-out ${
                isActive ? 'text-red-500' : 'text-zinc-500 hover:text-zinc-400'
              }`}
            >
              {item.icon}
